...
findPinnedAOD = async (amazonPage) => {
  try {
    ...
    await amazonPage.waitForXPath(
      '//*[@id="aod-pinned-offer-additional-content"]'
    );
    ...
    for (let AOD of pinnedAOD) {
      ...
      name = await AOD.$eval(
        "#aod-offer-soldBy > div > div >  div:nth-child(2) >a",
        (el) => el.innerText
      );
    }
...
checkIfAsinMatch = async (amazonPage, asin) => {
  try {
    const hiddenAsin = await amazonPage.evaluate(() => {
      const input = document.querySelector("#ddmSelectAsin");
      return input.value;
    });
    console.log(hiddenAsin);
    if (hiddenAsin == asin) {
      return true;
    } else {
      return false;
